 ...
 * Completely replaces WaterlooWorks job modal
 * Uses Gemini 2.5 Flash for intelligent content extraction
 */
...
(function() {
  ...
  const GEMINI_API_KEY = 'AIzaSyCHs5t-15gGXYdB_UR2YCKe_Q7EX9DtNGE';
  const GEMINI_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${GEMINI_API_KEY}`;

  // ============================================
  // IMMEDIATELY hide the original modal with CSS
  // This runs before any modal can open
  // ============================================
  
  const style = document.createElement('style');
  style.id = 'waw-modal-hide';
  style.textContent = `
    /* Hide the original WaterlooWorks modal container - it still renders but is invisible */
    ...
    /* Also hide the modal backdrop/overlay from WaterlooWorks */
    .modal__overlay, .modal-backdrop {
      background: transparent !important;
    }
    
    /* Prevent body scroll when our overlay is open */
  ...
  async function parseWithGemini(modalText, retryCount = 0) {
    const prompt = `You are extracting job posting information. From the following job posting text, extract ONLY these three fields and return them as valid JSON:

1. "description" - The job summary/description (what the job is about)
2. "responsibilities" - The job responsibilities (what the person will do)
3. "skills" - The required skills (what qualifications/skills are needed)
...
${modalText}`;
    ...
    try {
      ...
      console.log('[WAW] Gemini response:', result);
      ...
      console.log('[WAW] Parsed Gemini result:', parsed);
      return {
  ...
  // ============================================
  // Parse Basic Job Data (DOM selectors for reliable fields)
  // ============================================
  ...
  function parseBasicJobData(modal) {
    ...
    // Parse key-value lists for basic fields
    modal.querySelectorAll('.tag__key-value-list').forEach(kv => {
      const labelEl = kv.querySelector('.label');
      if (!labelEl) return;

      const label = labelEl.textContent.toLowerCase().trim().replace(/:$/, '');
      const pEl = kv.querySelector('p');
      if (!pEl) return;

      let value = pEl.textContent.trim();
      ...
      else if (label.includes('organization') && !data.company) data.company = value;
    });

    // Fallback deadline
    if (!data.deadline) {
      modal.querySelectorAll('.tag__key-value-list').forEach(kv => {
        const label = kv.querySelector('.label');
        if (label && label.textContent.toLowerCase().includes('deadline')) {
          const p = kv.querySelector('p');
          if (p) data.deadline = p.textContent.trim();
        }
      });
    }
  ...
  function showOverlay(data, isLoading = false) {
    ...
    const loadingHTML = `<div style="color:#718096;font-style:italic;">Loading content...</div>`;
  ...
  async function handleModal(modal) {
    ...
    const check = async () => {
      ...
      if ((hasTitle && hasFields) || attempts > 25) {
        ...
        // Get the full modal text for Gemini
        const modalText = modal.innerText || modal.textContent || '';
        
        // Call Gemini to parse description, responsibilities, skills
        const geminiData = await parseWithGemini(modalText);
        
        // Update overlay with Gemini's parsed content
  ...
  console.log('[WAW] Modal Redesign ready with Gemini integration');
})();
