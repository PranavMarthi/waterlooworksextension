...
(function() {
  ...
  // ============================================
  // Hide modal - just make it invisible, don't break WW's layout
  // ============================================
  
  function hideOriginalModal(el) {
    if (!el || el.id === 'waw-overlay') return;
    // Only hide visually - let WW manage its own positioning
    el.style.opacity = '0';
  ...
  const hideObserver = new MutationObserver(mutations => {
    for (const mut of mutations) {
      for (const node of mut.addedNodes) {
        ...
        if (node.matches?.('div[data-v-70e7ded6-s]')) {
          hideOriginalModal(node);
        }
        const modal = node.querySelector?.('div[data-v-70e7ded6-s]');
  ...
  document.querySelectorAll('div[data-v-70e7ded6-s]').forEach(hideOriginalModal);
  ...
  function showOverlay(data) {
    ...
    const closeAll = () => {
      ...
      // Keep WW modal invisible, but allow close click
      const wwModal = document.querySelector('div[data-v-70e7ded6-s]');
      if (wwModal) {
        wwModal.style.pointerEvents = 'auto';
        // Click the close button to let Vue handle cleanup properly
        const closeBtn = wwModal.querySelector('button[aria-label="Close"], .modal__close');
